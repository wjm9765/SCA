...
def check_adapter_config(checkpoint_dir: Path) -> bool:
    ...
    print(
        f"  PASS: Config valid (r={config.get('r')}, alpha={config.get('lora_alpha')})"
    )
...
def check_weights(checkpoint_dir: Path) -> bool:
    ...
    talker_lora_keys = [
        k for k in keys if "talker" in k and "lora_" in k and "code_predictor" not in k
    ]
    ...
    if len(thinker_lora_keys) == 0:
        ...
        if len(thinker_lora_keys) != expected_thinker_keys:
            print(
                f"  WARN: Expected {expected_thinker_keys} thinker LoRA keys, got {len(thinker_lora_keys)}"
            )
    ...
    if len(talker_lora_keys) == 0:
        ...
        if len(talker_lora_keys) != expected_talker_keys:
            print(
                f"  WARN: Expected {expected_talker_keys} talker LoRA keys, got {len(talker_lora_keys)}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate consolidated PEFT checkpoint"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
